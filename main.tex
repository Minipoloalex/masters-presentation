...
\begin{frame}{Instance Space Analysis: Example}
  \begin{columns}
    ...
    \begin{tabular}{@{}Xc@{}}
      \toprule
      \textbf{Algorithms} & \textbf{Success}
      \midrule
      CLK  & 0.2 \\
      LKCC & 0.8 \\
...
\begin{frame}{RL Algorithms}
  RL algorithms may support discrete (D) and/or continuous (C) actions.
  \begin{itemize}
    \item \textbf{A2C}: D/C
    \item \textbf{DQN}: D
    \item \textbf{PPO}: D/C
...
\section{Results}
\begin{frame}{Individual Environments}{Highway projection}
  \begin{columns}[c,onlytextwidth]
    \column{.55\textwidth}
      \footnotesize
      \[
...
\begin{frame}{Individual Environments}{Highway projection}
  ...
  \begin{tabular}{ccc}
    \includegraphics[width=.275\textwidth]{figures/results/single-envs/highway/2_lanes_count.pdf} &
    \includegraphics[width=.275\textwidth]{figures/results/single-envs/highway/2_traffic_density.pdf} &
    \includegraphics[width=.275\textwidth]{figures/results/single-envs/highway/2_baseline_speed_snr.pdf} \\
    \includegraphics[width=.275\textwidth]{figures/results/single-envs/highway/2_baseline_obs_volatility_mean.pdf} &
    \includegraphics[width=.275\textwidth]{figures/results/single-envs/highway/2_state_entropy.pdf} &
    \includegraphics[width=.275\textwidth]{figures/results/single-envs/highway/2_action_state_discontinuity_p95.pdf}
  \end{tabular}
  \endgroup
\end{frame}

\begin{frame}{Individual Environments}{Highway selector portfolio}
  \begin{columns}[c,onlytextwidth]
    \column{.58\textwidth}
      \centering
...
\begin{frame}{Multiple Environments}
\end{frame}

\begin{frame}{Domain-Specific Characteristics}
\end{frame}
...
\begin{frame}[fragile=singleslide]{Code is part of the slide}
  \begin{columns}[T,onlytextwidth]
    \column{.53\textwidth}
...
\begin{frame}[fragile=singleslide]{Longer code listing}{The title wraps without changing the code box}
...
\begin{frame}{Frame-local background images}{Full canvas with readable text}
  \begin{columns}[T,onlytextwidth]
    \column{.57\textwidth}
      \Large
